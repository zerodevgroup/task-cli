...
class Task {
  constructor() {
    ...
    if(!fs.existsSync(`${this.home}/tasks`)) {
      shell.mkdir('-p', `${this.home}/tasks`)
    }

    if(!fs.existsSync(`${this.home}/tasks/tasks.json`)) {
      shell.touch(`${this.home}/tasks/tasks.json`)
      shell.echo('[]').to(`${this.home}/tasks/tasks.json`)
    }
  }
